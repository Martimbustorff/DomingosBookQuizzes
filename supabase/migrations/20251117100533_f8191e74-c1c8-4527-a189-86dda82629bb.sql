-- Phase 1: Remove dangerous age defaults from books table
-- This prevents future Open Library imports from being auto-tagged as children's books
ALTER TABLE books 
  ALTER COLUMN age_min DROP DEFAULT,
  ALTER COLUMN age_max DROP DEFAULT;

-- Update the get_popular_books_dynamic() function to include age filtering
...
AS $function$
  ...
  WHERE b.title IS NOT NULL
    -- Safety filter: Only show children's books (age_max <= 12) OR manually added books without Open Library ID
